...
```python
model = scvi.model.JaxSCVI(adata)
```
...
```python
model_dir = os.path.join(save_dir.name, "scvi_jax_model")
model.save(model_dir, overwrite=True)
```

```python
model = scvi.model.JaxSCVI.load(model_dir, adata=adata)
```
...
SCVI_CLUSTERS_KEY = "leiden_scVI_jax"
...
```python
sc.tl.dendrogram(adata, groupby="cell_type", use_rep="X_scVI_jax")
```
