...
For a given cell x, the expression of the gene g is proportional to x_g = w\_(0, g) * z_0 + ... + w\_(9, g) * z_9. Moving from low values to high values in z_0 will mostly affect expression of genes with large w\_(0, :) weights. We can extract these weights from the `LDVAE` model, and identify which genes have high weights for each latent dimension.
