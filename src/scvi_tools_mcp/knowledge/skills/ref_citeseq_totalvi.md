...
rna_normalized, _ = model.get_normalized_expression(return_mean=True)
...
```python
# DE between clusters
de_rna = model.differential_expression(groupby="leiden", group1="0", group2="1")

# Filter significant genes
de_sig = de_rna[(de_rna["is_de_fdr_0.05"]) & (abs(de_rna["lfc_mean"]) > 1)]
...
```python
# Protein DE
de_protein = model.differential_expression(groupby="leiden", group1="0", group2="1", mode="protein")
...
for ax, protein in zip(axes, proteins_to_plot):
    ...
    sc.pl.umap(adata, color=adata.obsm["protein_denoised"][:, idx], ax=ax, title=protein, show=False)
plt.tight_layout()
```
...
sc.pl.dotplot(adata, var_names=de_sig.index[:20].tolist(), groupby="leiden", layer="totalVI_normalized")
```
...
```python
def analyze_citeseq(adata_rna, adata_protein, batch_key=None, n_top_genes=4000, n_latent=20):
    """
    ...
    adata.obsm["protein_expression"] = (
        adata_protein.X.toarray() if hasattr(adata_protein.X, "toarray") else adata_protein.X
    )
    adata.uns["protein_names"] = list(adata_protein.var_names)
    ...
    # HVG selection
    sc.pp.highly_variable_genes(adata, n_top_genes=n_top_genes, flavor="seurat_v3", batch_key=batch_key, layer="counts")
    adata = adata[:, adata.var["highly_variable"]].copy()

    # Setup totalVI
    scvi.model.TOTALVI.setup_anndata(
        adata, layer="counts", protein_expression_obsm_key="protein_expression", batch_key=batch_key
    )
...
# Usage
adata, model = analyze_citeseq(adata_rna, adata_protein, batch_key="batch")
